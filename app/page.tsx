...
import { AlgoVisualizer } from "@/components/algo-visualizer"
...
export default function CodingPlatform() {
  ...
  const challengeDetails: { [key: number]: any } = {
    1: {
      ...challenges[0],
      problemStatement:
  ...
  // Write your solution here
    ...
    2: {
      ...challenges[1],
      problemStatement:
        "Given a sorted array of integers, implement binary search to find the target value. Return the index if found, otherwise return -1.",
      exampleInput: "nums = [1,3,5,6,13,20], target = 13",
      exampleOutput: "4",
      testCases: [
        { input: "[1,3,5,6,13,20], 13", output: "4", hidden: false },
        { input: "[1,3,5,6,13,20], 0", output: "-1", hidden: false },
        { input: "[5], 5", output: "0", hidden: true },
      ],
      boilerplateCode: `function binarySearch(nums, target) {
  let left = 0;
  ...
  if (selectedChallengeId && challengeDetails[selectedChallengeId]) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100">
        ...
        <header className="border-b bg-white/80 backdrop-blur-sm sticky top-0 z-50">
          <div className="container mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              <Button variant="ghost" onClick={() => setSelectedChallengeId(null)} className="gap-2">
        ...
        <div className="container mx-auto px-4 py-8">
          <ChallengeEditor challenge={challengeDetails[selectedChallengeId]} />
        </div>
      </div>
    )
  }
          ...
          <TabsContent value="challenges" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {challenges.map((challenge) => (
                <Card
                  key={challenge.id}
                  className={`transition-all hover:shadow-lg cursor-pointer ${challenge.completed ? "bg-green-50 border-green-200" : challenge.locked ? "opacity-60" : "hover:scale-105"}`}
                  onClick={() => !challenge.locked && setSelectedChallengeId(challenge.id)}
                >
                  <CardHeader>
          ...
          <TabsContent value="visualizer" className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600 mt-1">Watch sorting and searching algorithms in action</p>
              </div>
            </div>
            <AlgoVisualizer />
          </TabsContent>
